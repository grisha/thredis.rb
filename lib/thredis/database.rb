require 'thredis/constants'
require 'thredis/errors'
require 'thredis/pragmas'
require 'thredis/statement'
require 'thredis/translator'
require 'thredis/value'

module Thredis

  # The Database class encapsulates a single connection to a Thredis database.
  # Its usage is very straightforward:
  #
  #   require 'thredis'
  #
  #   Thredis::Database.new( "data.db" ) do |db|
  ...
  # the Thredis data types (which are all represented as strings) may be
  ...
  class Database
    ...
    #
    def prepare sql
      stmt = Thredis::Statement.new( self, sql, true )
    ...
    def execute sql, bind_vars = [], *args, &block
      ...
      if bind_vars.nil? || !args.empty?
        ...
        warn(<<-eowarn)  if true
#{caller[0]} is calling Thredis::Database#execute with nil or multiple bind params
...
#{caller[0]} is calling Thredis::Database#execute_batch with bind parameters
...
array or hash. Support for this behavior will be removed in version 2.0.0.
      ...
      if bind_vars.nil? || !args.empty?
        ...
        warn(<<-eowarn) if $VERBOSE
#{caller[0]} is calling Thredis::Database#execute_batch with nil or multiple bind params
without using an array.  Please switch to passing bind parameters as an array.
Support for this behavior will be removed in version 2.0.0.
      ...
      until sql.empty? do
        prepare( sql ) do |stmt|
          # FIXME: this should probably use thredis's api for batch execution
    ...
    def query( sql, bind_vars = [], *args )
      ...
      if bind_vars.nil? || !args.empty?
        ...
        warn(<<-eowarn) if $VERBOSE
#{caller[0]} is calling Thredis::Database#query with nil or multiple bind params
without using an array.  Please switch to passing bind parameters as an array.

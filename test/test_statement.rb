...
module Thredis
  class TestStatement < Thredis::TestCase
    def setup
      @db   = Thredis::Database.new(THREDIS_CONNECT)
      @stmt = Thredis::Statement.new(@db, "select 'foo'")
      @db.execute('drop table if exists foo')
    end

    def test_raises_type_error
      assert_raises(TypeError) do
        Thredis::Statement.new( @db, nil )
      end
    end

    ###
    # This method may not exist depending on how thredis was compiled
    ...
    def test_new_closed_handle
      ...
      assert_raises(ArgumentError) do
        Thredis::Statement.new(@db, 'select "foo"')
      end
    end
    ...
    def test_double_close
      @stmt.close
      assert_raises(Thredis::Exception) do
        @stmt.close
      end
    end

    def test_bind_param_string
      stmt = Thredis::Statement.new(@db, "select ?")
      stmt.bind_param(1, "hello")
      result = nil
    ...
    def test_with_error
      ...
      stmt.execute('employee-1') rescue Thredis::ConstraintException
      stmt.reset!
      assert_nothing_raised(Thredis::ConstraintException) {
    ...
    def test_clear_bindings
      ...
      # We can't fetch bound parameters back out of thredis, so just call
